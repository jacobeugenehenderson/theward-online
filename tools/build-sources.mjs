...
let index
try { index = readFileSync(indexPath, 'utf8') } catch { die('no index.html to write into') }
const a = index.indexOf(BEGIN), b = index.indexOf(END)
if (a < 0 || b < 0) die(`index.html is missing the SOURCES markers:\n    ${BEGIN}\n    ${END}`)

const next = index.slice(0, a + BEGIN.length) + '\n' + html + '\n        ' + index.slice(b)

if (CHECK) {
  if (next !== index) die('index.html sources block is STALE. Run: node tools/build-sources.mjs')
