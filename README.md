...
handbook/           the Local Host Handbook — for whoever brings a Ward somewhere.
...
| The Slab | ⛔ **NO LONGER A CHAPTER, AND NO LONGER A WORD ON THE PAGE (2026-09-05).** It had a chapter for a day. What a reader needs to know about a Slab is the thing that makes a Ward portable, so it became *Built to travel* in §05 Bring It Somewhere — and then that paragraph was rewritten to drop the term entirely, along with *Universal Player*. ⭐ **Why, and it governs copy edits generally**: naming the internal parts asks a reader to hold a vocabulary before they have a reason to want one, and defining what a thing IS before saying what it does reads as heavy. The site's reader is deciding whether to care, not whether to build. The precision belongs here and in `INTEGRATION.md`, where the audience is the next builder |
| §04 How It Comes Together | ⛔ **AND ITS TABLE HAS LEFT.** The sources block moved to `/works` on 2026-09-05 (Jacob: *"I think it goes better in this milieu anyway"*) — a fourteen-row provenance audit is evidence in a technical prospectus, and on a page written for someone deciding whether to care it was the most technical object in the middle of the pitch. ⚠️ **What is left is two paragraphs**, which is thin for a chapter the Field Guide promises. Either it earns more, or it folds back into §05 and the guide goes to five. ⛔ **No `<h2>`** — the plate says *How It Comes Together* and the heading said *Where a Ward Comes From*, the same sentence twice in two type sizes. ⚠️ The instruments paragraph here is the site's only mention of the Surveyor, Arborist, Meteorologist and Designer |
...
| A privacy page | ⛔ **RULED OUT, 2026-08-31** (Jacob: *"this page doesn't get a privacy statement"*). One was written and reverted whole. **The PRODUCT already has one** — `PrivacyPage` in `src/pages/LegalPage.jsx` — and everything load-bearing in the draft described the thing in the FRAME, not this website, which collects nothing and needs about two sentences to say so. A second copy on a second domain drifts the first time either moves. ⚠️ It also promised on behalf of *every* Ward, when a Local Host's installation is not ours to speak for — which `legal.html §1` expressly disclaims. ▶ A mission-style statement about earning standing by presence is **parked, not pursued**; it is a copy decision. `BACKLOG.md` carries the full reasoning |
...
## 7d. ⛔ The Local Host Handbook carries no rate
...
travel together. A findable document is right — the front page recruits Local Hosts in
...
can act on it, and this document's reader is a prospective Local Host. ⚠️ **So this
